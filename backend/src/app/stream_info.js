...
var StreamInfo = Backbone.Model.extend({
  ...
  shallow: function() {
    this.updateStats();
    return {
      downloaded: this.get('downloaded'),
      active_peers: this.get('active_peers'),
      total_peers: this.get('total_peers'),
      uploadSpeed: this.get('uploadSpeed'),
      downloadSpeed: this.get('downloadSpeed'),
